...
-- 1. Temporary table for individual dataset scores
CREATE TEMP TABLE dataset_scores AS
...
raw_scores AS (
    ...
    WHERE d.deleted IS FALSE AND d.published IS TRUE
)
SELECT
    direction,
    quality_score,
...
-- 2. Aggregate and Insert into read-model
BEGIN;
DELETE FROM direction_health_stats;

INSERT INTO direction_health_stats (
    direction,
    score_quality,
    score_freshness,
    score_engagement,
...
SELECT
    ...
    ROUND(AVG(freshness_score)::numeric, 2),
    ROUND(AVG(engagement_score)::numeric, 2),
    ROUND(AVG(global_score)::numeric, 2),
    COUNT(*),
    COUNT(*) FILTER (WHERE global_score < 50)
FROM dataset_scores
GROUP BY direction;
COMMIT;

-- Cleanup
DROP TABLE dataset_scores;
