...
let API_ORIGIN = rawApiUrl
  ? `${rawApiUrl.replace(/\/$/, "")}${apiPort ? `:${apiPort}` : ""}`
  : undefined;
let API_PATH_PREFIX = "";
...
if (!API_ORIGIN) {
  ...
  console.error(
    "Missing API origin. Set API_URL (+ optional API_PORT) or API_TARGET as full URL."
  );
  process.exit(1);
}
...
app.listen(PORT, () => {
  // eslint-disable-next-line no-console
  console.log(
    `Proxy listening on http://localhost:${PORT} -> ${API_ORIGIN}${
      API_PATH_PREFIX ? ` (rewrite ^/api -> ${API_PATH_PREFIX})` : ""
    }`
  );
});
