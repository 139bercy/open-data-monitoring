...
describe('API Client - Robustness Tests', () => {
    ...
    describe('syncDatasetFromSource', () => {
        const testUrl = 'https://data.economie.gouv.fr/explore/dataset/test-sync/information/';
        ...
        it('should call the correct endpoint with encoded URL', async () => {
            ...
            server.use(
                http.post('/api/datasets/add', ({ request }) => {
                    ...
                    return HttpResponse.json({ status: 'success', dataset_id: 'test-sync' });
                })
            );
        ...
        it('should handle malformed ODS URLs that might cause fetch failures', async () => {
            const malformedUrl = 'https://data.economie.gouv.fr/explore/dataset/invalid-id/information/';
            ...
            await expect(syncDatasetFromSource(malformedUrl)).rejects.toThrow('HTTP 404');
        });
    });
});
