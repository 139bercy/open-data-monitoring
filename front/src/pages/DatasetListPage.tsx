...
export function DatasetListPage(): JSX.Element {
    ...
    const [data, setData] = useState<PaginatedResponse<DatasetSummary>>({ items: [], total: 0, page: 1, pageSize: 25 });
    ...
    useEffect(() => {
        getPlatforms().then(setPlatforms).catch(() => undefined);
    }, []);

    // reload publishers when platform changes (basic)
    useEffect(() => {
        const platformId = query.platformId;
        getPublishers({ platformId, limit: 200 }).then(setPublishers).catch(() => setPublishers([]));
    }, [query.platformId]);
    ...
    useEffect(() => {
        ...
        getDatasets(query)
            .then(res => {
                ...
                const totalPages = Math.max(1, Math.ceil((res.total ?? 0) / Math.max(1, (query.pageSize ?? 25))));
        ...
        query.page, query.pageSize, query.sortBy, query.order,
        query.platformId, query.publisher,
    ...
    return (
        <div className="fr-container fr-my-6w">
            ...
            <DatasetTable
                ...
                sortBy={query.sortBy}
                order={query.order}
                onSortChange={(sortBy, order) => setQuery(q => ({ ...q, sortBy, order, page: 1 }))}
