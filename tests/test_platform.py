...
def test_create_platform(platform_app, testfile):
    # Arrange & Act
...
def test_api_key_should_be_hidden(platform_app, testfile):
    # Arrange & Act
    platform_id = create_platform(platform_app, platform_1)
    # Assert
    platform = platform_app.get(platform_id=platform_id)
...
def test_should_return_all_platforms(platform_app: PlatformMonitoring, testfile):
    # Arrange
    create_platform(platform_app, platform_1)
    # Act
    result = platform_app.get_all_platforms()
    # Assert
    assert len(result) == 1
...
def test_get_platform_by_domain(platform_app, testfile):
    # Arrange & Act
    create_platform(platform_app, platform_1)
...
def test_sync_platform(platform_app):
    # Arrange
    platform_id = create_platform(platform_app, platform_1)
    # Act
    sync_platform(platform_app, platform_id=platform_id)
    # Assert
...
def test_retrieve_platform_with_syncs(platform_app):
    # Arrange
    platform_id = create_platform(platform_app, platform_1)
